    ...
    class Validator
      ...
      def validate(css_text, profile: 'css3svg', output_format: 'text')
        ...
        Tempfile.create(['css_validator', '.css']) do |tempfile|
          ...
          command = build_command(tempfile.path, profile, output_format)
      ...
      def validate_file(file_path, profile: 'css3svg', output_format: 'text')
        ...
        command = build_command(file_path, profile, output_format)
        stdout, stderr, status = Open3.capture3(command)
      ...
      def build_command(file_path, profile, output_format)
        "java -jar #{@jar_path} --output=#{output_format} --profile=#{profile} file://#{file_path}"
      end
    end
  end
end
